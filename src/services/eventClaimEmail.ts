...
import { EmailResult, EventClaimAttachment } from '../types';
...
const fileToBase64 = async (attachment: EventClaimAttachment): Promise<string> => {
  if (Platform.OS === 'web') {
...
export const sendEventClaimEmail = async (payload: {
  ...
  socialHandle?: string;
  note?: string;
  attachments: EventClaimAttachment[];
}): Promise<EmailResult> => {
  if (!ORGANIZER_COMPLIANCE_WEBHOOK_URL) {
    return {
      sent: true,
      mode: 'simulated',
      detail: 'Webhook review non configurato.',
    };
  }
  ...
  try {
    ...
    return {
      sent: false,
      mode: 'webhook',
      detail: 'Errore rete invio claim evento.',
    };
  }
};
